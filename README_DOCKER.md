...
The MTL LDNS docker image is dependent on the MTL Mode base image which consists of MTL Mode (version >= 1.3.0), OpenSSL (version 3.5.0+), and LibOQS (version 0.16.0+).

That base image can be built using the MTL repository: (https://github.com/verisign/MTL) using branch 1.3.0
...
Assuming a local directory ```zones``` with the zone files to sign/verify the following command can be used to start the container placing the zone files in ```/var/dns/zones```
