...
#if defined(PQC_ALGO_FL_DSA) || defined(PQC_ALGO_ML_DSA) || defined(PQC_ALGO_SLH_DSA_SHA2) || defined(PQC_ALGO_SLH_DSA_SHAKE) || defined(PQC_ALGO_MAYO_1) || defined(PQC_ALGO_MAYO_2) || defined(PQC_ALGO_SNOVA) || defined(PQC_ALGO_MTL)
#include <oqs/sig.h>
#endif
...
ldns_dnssec_mtl_ladder_cache_init(uint32_t cache_size)
{
    if(ladder_cache == NULL) {
        ladder_cache = calloc(1, sizeof(ldns_dnssec_mtl_ladder_cache));
        if(ladder_cache == NULL) {
            fprintf(stderr, "Unable to allocate the ladder cache\n");
        }
...
ldns_dnssec_mtl_ladder_cache_insert(LADDER* ladder) {
    if(ladder_cache == NULL) {
        fprintf(stderr, "Ladder Cache is NULL (not initalized)\n");
        return;
    }
...
/**
 * Find the best ladder in cache for an auth path
 *
 * \param[in] auth path that will be verified
 * \return Ladder to use to verify the auth path
 */
LADDER* ldns_dnssec_mtl_ladder_find_ladder(SERIESID* sid, uint32_t leaf_index) {
    LADDER* target = NULL;
    uint32_t target_span = 0;

    for(uint32_t l=0; l<ladder_cache->max_entries; l++)
    {
        if(ladder_cache->cache[l] != NULL)
        {
            if((ladder_cache->cache[l]->sid.length == sid->length) && 
                (memcmp(ladder_cache->cache[l]->sid.id, sid->id, ladder_cache->cache[l]->sid.length) == 0))
            {
                ...
                for(uint16_t r=0; r<ladder_cache->cache[l]->rung_count; r++)
                {
                    ...
                    if((leaf_index >= cache_rung[r].left_index) && 
                       (leaf_index <= cache_rung[r].right_index)) {
 ...
 */
RUNG* ldns_dnssec_mtl_ladder_find_rung(AUTHPATH* path) {
    RUNG* target = NULL;
    uint32_t target_span = 0;
    ...
    ldns_buffer* rrset, unsigned char* key, size_t keylen, uint8_t algo)
{
    ...
    if (keystr == NULL) {
            ...
            fprintf(stderr, "Unknown MTL signature scheme\n");
            result = LDNS_STATUS_CRYPTO_BOGUS;                                           
    }

    hash_size = mtllib_sig_buffer_get_hash_size(keystr);
    ...
    // remove leading byte
    signature = &sig[1];
    signature_len = siglen-1;

    if (mtllib_buffer_initialize(&signature_buff, signature_len, signature) != MTLLIB_OK) {
        return LDNS_STATUS_INTERNAL_ERR;
    }

    if(mtllib_sig_buffer_get_sid(signature_buff, hash_size, &sid) != MTLLIB_OK) {
        mtllib_buffer_free(signature_buff);        
        return LDNS_STATUS_INTERNAL_ERR;
    }

    if(mtllib_sig_buffer_get_leaf_index(signature_buff, hash_size, &leaf_index) != MTLLIB_OK) {
        mtllib_buffer_free(signature_buff);        
        return LDNS_STATUS_INTERNAL_ERR;
    }

    // Setup the MTL CTX from the key information
    MTLLIB_BUFFER* pubkey = NULL;
    if(mtllib_buffer_initialize(&pubkey, keylen, key) != MTLLIB_OK) {
        result = LDNS_STATUS_CRYPTO_NO_DNSKEY;
    } else if(mtllib_pubkey_from_buffer(keystr, &mtl_ctx, pubkey, sid.id) != MTLLIB_OK) {
        result = LDNS_STATUS_CRYPTO_NO_DNSKEY;
    }
    mtllib_buffer_free(pubkey);

    // Check cache for a useable rung
    LADDER* mtl_ladder = ldns_dnssec_mtl_ladder_find_ladder(&sid, leaf_index);
    uint8_t* mtl_ladder_buffer = NULL;
    size_t mtl_ladder_len = 0;
    MTLLIB_BUFFER* ladder_buffer = NULL;
    if(mtl_ladder != NULL) {
        mtl_ladder_len = mtl_ladder_to_buffer(mtl_ladder, hash_size, &mtl_ladder_buffer);

        if(mtllib_buffer_initialize(&ladder_buffer, mtl_ladder_len, mtl_ladder_buffer) != MTLLIB_OK) {
            mtllib_buffer_free(signature_buff);        
            return LDNS_STATUS_INTERNAL_ERR;
        }        
    }

    MTLLIB_BUFFER* rrset_buffer = NULL;
    if(mtllib_buffer_initialize(&rrset_buffer, ldns_buffer_position(rrset), ldns_buffer_begin(rrset))) {
        mtllib_buffer_free(signature_buff);        
        mtllib_buffer_free(ladder_buffer);
        return LDNS_STATUS_INTERNAL_ERR;
    }

    verify_result = mtllib_verify(mtl_ctx,
                                  rrset_buffer,
                                  signature_buff,
                                  ladder_buffer,
                                  &condensed_sig_len);
    mtllib_buffer_free(ladder_buffer);
    mtllib_buffer_free(rrset_buffer);
    if(verify_result == MTLLIB_OK_VALIDATED_LADDER) {
        // Cache this ladder
        LADDER* full_ladder = NULL;
        mtl_ladder_from_buffer((uint8_t*)&signature_buff->buffer_data[condensed_sig_len],signature_len-condensed_sig_len, hash_size, &full_ladder);
        ldns_dnssec_mtl_ladder_cache_insert(full_ladder);
        // Only a reference to the ladder is added to the cache so do not free it here, it will be freed on eviction from cache.
    }
    else if(verify_result == MTLLIB_OK) {
        // Nothing to do as signature was validated from cache
    } else {
        result = LDNS_STATUS_CRYPTO_BOGUS;
    }
    mtllib_buffer_free(signature_buff);
    free(mtl_ladder_buffer);
...
ldns_verify_rrsig_mtl_ladder(unsigned char* sig, size_t siglen,
    ...
    MTLLIB_STATUS verify_result = MTLLIB_OK;
    size_t offset = 0;
    MTLLIB_BUFFER* sign_ladder_buffer = NULL;
    ...
    // Find the SID from the signature
    signature = &sig[1];
    signature_len = siglen-1;
    ...
    // The ladder is appended after the condensed signature, so get the condensed signature length
    offset = mtllib_sig_buffer_condensed_sig_len(signature_buff, hash_size);

    // Verify the ladder which is in the signature buffer right after the condensed signature 
    if(mtllib_buffer_initialize(&sign_ladder_buffer, signature_len - offset, &signature_buff->buffer_data[offset]) == MTLLIB_OK) {
        verify_result = mtllib_verify_signed_ladder(mtl_ctx, sign_ladder_buffer);

        if(verify_result != MTLLIB_OK) {
            result = LDNS_STATUS_CRYPTO_BOGUS;
        } else {
            LADDER* full_ladder = NULL;
            mtl_ladder_from_buffer((uint8_t*)&signature_buff->buffer_data[offset], signature_len - offset, hash_size, &full_ladder);
            ldns_dnssec_mtl_ladder_cache_insert(full_ladder);
        }
    } else {
        result = LDNS_STATUS_CRYPTO_BOGUS;
    }
    mtllib_buffer_free(sign_ladder_buffer);
    mtllib_buffer_free(signature_buff);
    mtllib_key_free(mtl_ctx);
    return result;
}
                        ...
                        size_t keylen, uint8_t algo)
{
    ...
    case LDNS_SIGN_FL_DSA_512:
        ...
        return ldns_verify_rrsig_oqs_raw(sig, siglen, verify_buf,
                                         key, (char*)PQC_ALGO_FL_DSA_SCHEME);
#endif
#ifdef PQC_ALGO_ML_DSA
    case LDNS_SIGN_ML_DSA_44:
        // ML-DSA
        return ldns_verify_rrsig_oqs_raw(sig, siglen, verify_buf,
                                         key, (char*)PQC_ALGO_ML_DSA_SCHEME);
#endif
