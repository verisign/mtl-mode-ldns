...
#if defined(NEED_OQS_PIGGYBACK) || defined(PQC_ALGO_FL_DSA) || defined(PQC_ALGO_ML_DSA) || defined(PQC_ALGO_SLH_DSA_SHA2) || defined(PQC_ALGO_SLH_DSA_SHAKE) || defined(PQC_ALGO_MTL) || defined(PQC_ALGO_MAYO_1) || defined(PQC_ALGO_MAYO_2) || defined(PQC_ALGO_SNOVA)
#include <oqs/sig.h>
#endif
...
ldns_lookup_table ldns_signing_algorithms[] = {
        ...
        { LDNS_SIGN_SLH_DSA_SHA2_128s, "SLH_DSA_PURE_SHA2_128S"},
#endif
#ifdef PQC_ALGO_SLH_DSA_SHAKE
        { LDNS_SIGN_SLH_DSA_SHAKE_128s, "SLH_DSA_PURE_SHAKE_128S"},
#endif        		
...
#endif
#ifdef PQC_ALGO_FL_DSA_MTL_SHAKE
		{ LDNS_SIGN_FL_DSA_512_MTL_SHAKE_128, "Falcon-padded-512-MTL-SHAKE-128"},
#endif
#ifdef PQC_ALGO_ML_DSA_MTL_SHAKE
        { LDNS_SIGN_ML_DSA_44_MTL_SHAKE_128, "ML-DSA-44-MTL-SHAKE-128"},
#endif
#ifdef PQC_ALGO_SLH_DSA_SHA2_MTL_SHA2 
        { LDNS_SIGN_SLH_DSA_SHA2_128s_MTL_SHA2_128, "SLH-DSA-SHA2-128s-MTL-SHA2-128"},
#endif
#ifdef PQC_ALGO_SLH_DSA_SHAKE_MTL_SHAKE
        { LDNS_SIGN_SLH_DSA_SHAKE_128s_MTL_SHAKE_128, "SLH-DSA-SHAKE-128s-MTL-SHAKE-128"},
#endif
#ifdef PQC_ALGO_MAYO_1_MTL_SHAKE
		{ LDNS_SIGN_MAYO_1_MTL_SHAKE_128, "MAYO-1-MTL-SHAKE-128"},
#endif
#ifdef PQC_ALGO_MAYO_2_MTL_SHAKE
		{ LDNS_SIGN_MAYO_2_MTL_SHAKE_128, "MAYO-2-MTL-SHAKE-128"},
#endif
#ifdef PQC_ALGO_SNOVA_MTL_SHAKE
		{ LDNS_SIGN_SNOVA_24_5_4_MTL_SHAKE_128, "SNOVA_24_5_4-MTL-SHAKE-128"},
#endif
        { 0, NULL }
};
...
ldns_key_new_frm_fp_oqs_l(FILE* fp, char* algorithm) {
    ...
    priv_key = calloc(1,sizeof(oqs_key));
...
#ifdef PQC_ALGO_MTL
static MTLLIB_CTX*
ldns_key_new_frm_fp_mtl_l(FILE* fp) {
    char key_str[LDNS_MAX_PACKETLEN];
    ldns_rdf* b64rdf = NULL;
	MTLLIB_CTX *mtl_ctx = NULL;
	MTLLIB_BUFFER *b64rdf_buffer = NULL;
	...
	if(mtllib_buffer_initialize(&b64rdf_buffer, ldns_rdf_size(b64rdf), ldns_rdf_data(b64rdf)) != MTLLIB_OK) {
		ldns_rdf_deep_free(b64rdf);
		return NULL;
	}
	if(mtllib_key_from_buffer(b64rdf_buffer, &mtl_ctx) != MTLLIB_OK) {
		ldns_rdf_deep_free(b64rdf);
		mtllib_buffer_free(b64rdf_buffer);
		return NULL;
	}
	ldns_rdf_deep_free(b64rdf);
	mtllib_buffer_free(b64rdf_buffer);
...
ldns_key_new_frm_fp_l(ldns_key **key, FILE *fp, int *line_nr)
{
	...
	ldns_rr *key_rr;
#if defined(PQC_ALGO_MTL) || defined (PQC_ALGO_FL_DSA) || defined (PQC_ALGO_ML_DSA) || defined (PQC_ALGO_SLH_DSA_SHA2) || defined (PQC_ALGO_SLH_DSA_SHAKE) || defined (PQC_ALGO_MAYO_1) || defined(PQC_ALGO_MAYO_2) || defined (PQC_ALGO_SNOVA) || defined (PQC_ALGO_SQISIGN) || defined (PQC_ALGO_HAWK)
    char alg_id[5];	
#endif
#ifdef HAVE_SSL
	RSA *rsa;
...
#endif
#ifdef PQC_ALGO_FL_DSA_MTL_SHAKE
    snprintf(&alg_id[0], 5, "%d ", LDNS_SIGN_FL_DSA_512_MTL_SHAKE_128);
    if (strncmp(d, alg_id, strlen(alg_id)) == 0) {    
        alg = LDNS_SIGN_FL_DSA_512_MTL_SHAKE_128;
    }
#endif
#ifdef PQC_ALGO_ML_DSA_MTL_SHAKE
    snprintf(&alg_id[0], 5, "%d ", LDNS_SIGN_ML_DSA_44_MTL_SHAKE_128);
    if (strncmp(d, alg_id, strlen(alg_id)) == 0) {    
        alg = LDNS_SIGN_ML_DSA_44_MTL_SHAKE_128;
    }
#endif
#ifdef PQC_ALGO_SLH_DSA_SHA2_MTL_SHA2
    snprintf(&alg_id[0], 5, "%d ", LDNS_SIGN_SLH_DSA_SHA2_128s_MTL_SHA2_128);
    if (strncmp(d, alg_id, strlen(alg_id)) == 0) {    
        alg = LDNS_SIGN_SLH_DSA_SHA2_128s_MTL_SHA2_128;
    }   
#endif
#ifdef PQC_ALGO_SLH_DSA_SHAKE_MTL_SHAKE
    snprintf(&alg_id[0], 5, "%d ", LDNS_SIGN_SLH_DSA_SHAKE_128s_MTL_SHAKE_128);
    if (strncmp(d, alg_id, strlen(alg_id)) == 0) {    
        alg = LDNS_SIGN_SLH_DSA_SHAKE_128s_MTL_SHAKE_128;
    }       
#endif        	
#ifdef PQC_ALGO_MAYO_1_MTL_SHAKE
	snprintf(&alg_id[0], 5, "%d ", LDNS_SIGN_MAYO_1_MTL_SHAKE_128);
	if (strncmp(d, alg_id, strlen(alg_id)) == 0) {
		alg = LDNS_SIGN_MAYO_1_MTL_SHAKE_128;
	}
#endif
#ifdef PQC_ALGO_MAYO_2_MTL_SHAKE
	snprintf(&alg_id[0], 5, "%d ", LDNS_SIGN_MAYO_2_MTL_SHAKE_128);
	if (strncmp(d, alg_id, strlen(alg_id)) == 0) {
		alg = LDNS_SIGN_MAYO_2_MTL_SHAKE_128;
	}
#endif
#ifdef PQC_ALGO_SNOVA_MTL_SHAKE
	snprintf(&alg_id[0], 5, "%d ", LDNS_SIGN_SNOVA_24_5_4_MTL_SHAKE_128);
	if (strncmp(d, alg_id, strlen(alg_id)) == 0) {
		alg = LDNS_SIGN_SNOVA_24_5_4_MTL_SHAKE_128;
	}
#endif
#ifdef PQC_ALGO_SQISIGN_MTL_SHAKE
	snprintf(&alg_id[0], 5, "%d ", LDNS_SIGN_SQISIGN_LVL1_MTL_SHAKE_128);
	if (strncmp(d, alg_id, strlen(alg_id)) == 0) {
		alg = LDNS_SIGN_SQISIGN_LVL1_MTL_SHAKE_128;
	}
#endif
#ifdef PQC_ALGO_HAWK_MTL_SHAKE
	snprintf(&alg_id[0], 5, "%d ", LDNS_SIGN_HAWK_512_MTL_SHAKE_128);
	if (strncmp(d, alg_id, strlen(alg_id)) == 0) {
		alg = LDNS_SIGN_HAWK_512_MTL_SHAKE_128;
	}
#endif
	LDNS_FREE(d);
	...
	switch(alg) {
        ...
        case LDNS_SIGN_FL_DSA_512:
            ...
            ldns_key_set_external_key(k,
                    ldns_key_new_frm_fp_oqs_l(fp, (char*) PQC_ALGO_FL_DSA_SCHEME));
            break;
#endif
#ifdef PQC_ALGO_ML_DSA
        case LDNS_SIGN_ML_DSA_44:
            ldns_key_set_algorithm(k, alg);
            ldns_key_set_external_key(k,
                    ldns_key_new_frm_fp_oqs_l(fp, (char*) PQC_ALGO_ML_DSA_SCHEME));
            break;
#endif
#ifdef PQC_ALGO_SLH_DSA_SHA2 
...
#ifdef PQC_ALGO_MTL
	#ifdef PQC_ALGO_FL_DSA_MTL_SHAKE
    case LDNS_SIGN_FL_DSA_512_MTL_SHAKE_128:
	#endif
	#ifdef PQC_ALGO_ML_DSA_MTL_SHAKE
    case LDNS_SIGN_ML_DSA_44_MTL_SHAKE_128:
	#endif
	#ifdef PQC_ALGO_SLH_DSA_SHA2_MTL_SHA2
    case LDNS_SIGN_SLH_DSA_SHA2_128s_MTL_SHA2_128:
	#endif
	#ifdef PQC_ALGO_SLH_DSA_SHAKE_MTL_SHAKE
    case LDNS_SIGN_SLH_DSA_SHAKE_128s_MTL_SHAKE_128:
	#endif
	#ifdef PQC_ALGO_MAYO_1_MTL_SHAKE
	case LDNS_SIGN_MAYO_1_MTL_SHAKE_128:
	#endif
	#ifdef PQC_ALGO_MAYO_2_MTL_SHAKE
	case LDNS_SIGN_MAYO_2_MTL_SHAKE_128:
	#endif
	#ifdef PQC_ALGO_SNOVA_MTL_SHAKE
...
ldns_key_new_custom_frm_algorithm(char* algorithm) {
	...
	if (strncmp(algorithm, PQC_ALGO_SQISIGN_SCHEME, strlen(PQC_ALGO_SQISIGN_SCHEME)) == 0) {
		...
		if (key_pair != NULL) {
			...
			key_pair->alg_id = calloc(strlen(PQC_ALGO_SQISIGN_SCHEME) + 1, 1);
		}
	...
	if (strncmp(algorithm, PQC_ALGO_HAWK_SCHEME, strlen(PQC_ALGO_HAWK_SCHEME)) == 0) {
		...
		if (key_pair != NULL) {
			...
			key_pair->alg_id = calloc(strlen(PQC_ALGO_HAWK_SCHEME)+1, 1);
		}
		if ((key_pair == NULL) || (key_pair->pk == NULL) || (key_pair->sk == NULL) || (key_pair->alg_id == NULL)) {
			fprintf(stderr, "ERROR Unable to allocate memory\n");
			if ((key_pair != NULL) && (key_pair->pk != NULL)) {
...
# endif
...
#ifdef PQC_ALGO_FL_DSA
        case LDNS_SIGN_FL_DSA_512:
            new_oqs_key = ldns_key_new_oqs_frm_algorithm((char*)PQC_ALGO_FL_DSA_SCHEME);
            ldns_key_set_external_key(k, new_oqs_key);
            break;
#endif
#ifdef PQC_ALGO_ML_DSA
        case LDNS_SIGN_ML_DSA_44:
            new_oqs_key = ldns_key_new_oqs_frm_algorithm((char*)PQC_ALGO_ML_DSA_SCHEME);
            ldns_key_set_external_key(k, new_oqs_key);
            break;
#endif
#ifdef PQC_ALGO_SLH_DSA_SHA2 
	...
	#ifdef PQC_ALGO_FL_DSA_MTL_SHAKE
		case LDNS_SIGN_FL_DSA_512_MTL_SHAKE_128:
			if(mtllib_key_new((char*)PQC_ALGO_FL_DSA_MTL_SHAKE_SCHEME, &mtllib_ctx) != MTLLIB_OK) {
				ldns_key_free(k);
                return NULL;				
			}
            ldns_key_set_external_key(k, mtllib_ctx);
            break;
	#endif
	#ifdef PQC_ALGO_ML_DSA_MTL_SHAKE
  		case LDNS_SIGN_ML_DSA_44_MTL_SHAKE_128:
			if(mtllib_key_new((char*)PQC_ALGO_ML_DSA_MTL_SHAKE_SCHEME, &mtllib_ctx) != MTLLIB_OK) {
				ldns_key_free(k);
                return NULL;				
			}
            ldns_key_set_external_key(k, mtllib_ctx);
	...
	#ifdef PQC_ALGO_SLH_DSA_SHA2_MTL_SHA2
		case LDNS_SIGN_SLH_DSA_SHA2_128s_MTL_SHA2_128:
			if(mtllib_key_new((char*)PQC_ALGO_SLH_DSA_SHA2_MTL_SHA2_SCHEME, &mtllib_ctx) != MTLLIB_OK) {
				ldns_key_free(k);
                return NULL;				
			}
            ldns_key_set_external_key(k, mtllib_ctx);
	...
	#ifdef PQC_ALGO_SLH_DSA_SHAKE_MTL_SHAKE
		case LDNS_SIGN_SLH_DSA_SHAKE_128s_MTL_SHAKE_128:
			if(mtllib_key_new((char*)PQC_ALGO_SLH_DSA_SHAKE_MTL_SHAKE_SCHEME, &mtllib_ctx) != MTLLIB_OK) {
				ldns_key_free(k);
                return NULL;				
			}
            ldns_key_set_external_key(k, mtllib_ctx);
	...
	#ifdef PQC_ALGO_MAYO_1_MTL_SHAKE
		case LDNS_SIGN_MAYO_1_MTL_SHAKE_128:
			if(mtllib_key_new((char*)PQC_ALGO_MAYO_1_MTL_SHAKE_SCHEME, &mtllib_ctx) != MTLLIB_OK) {
				ldns_key_free(k);
                return NULL;				
			}
            ldns_key_set_external_key(k, mtllib_ctx);
	...
	#ifdef PQC_ALGO_MAYO_2_MTL_SHAKE
		case LDNS_SIGN_MAYO_2_MTL_SHAKE_128:
			if(mtllib_key_new((char*)PQC_ALGO_MAYO_2_MTL_SHAKE_SCHEME, &mtllib_ctx) != MTLLIB_OK) {
				ldns_key_free(k);
                return NULL;				
			}
            ldns_key_set_external_key(k, mtllib_ctx);
		...
		#ifdef PQC_ALGO_SNOVA_MTL_SHAKE
		case LDNS_SIGN_SNOVA_24_5_4_MTL_SHAKE_128:
			if(mtllib_key_new((char*)PQC_ALGO_SNOVA_MTL_SHAKE_SCHEME, &mtllib_ctx) != MTLLIB_OK) {
				ldns_key_free(k);
                return NULL;				
			}
            ldns_key_set_external_key(k, mtllib_ctx);
...
ldns_key_free_from_mtl_algorithm(ldns_key *k) {
	MTLLIB_CTX* mtllib_key_ptr = (MTLLIB_CTX*)ldns_key_external_key(k);
	if(mtllib_key_ptr != NULL) {
		mtllib_key_free(mtllib_key_ptr);
	}
}
#endif
...
#ifdef PQC_ALGO_MTL
uint8_t ldns_key_algorithm_is_mtl(const ldns_key *k)
{
	switch (k->_alg) {
		#ifdef PQC_ALGO_FL_DSA_MTL_SHAKE
		case LDNS_SIGN_FL_DSA_512_MTL_SHAKE_128:
...
ldns_key2rr(const ldns_key *k)
{
	...
	int internal_data = 0;
#ifdef PQC_ALGO_MTL
	MTLLIB_CTX* pub_mtllib_key = NULL;	
#endif
#if defined(PQC_ALGO_FL_DSA) || defined (PQC_ALGO_ML_DSA) || defined(PQC_ALGO_SLH_DSA_SHA2) || defined(PQC_ALGO_SLH_DSA_SHAKE) || defined(PQC_ALGO_MAYO_1) || defined(PQC_ALGO_MAYO_2) || defined(PQC_ALGO_SNOVA)
	...
	switch(ldns_key_algorithm(k)) {
		...
		case LDNS_SIGN_SNOVA_24_5_4_MTL_SHAKE_128:
            ...
            pub_mtllib_key = (MTLLIB_CTX*)ldns_key_external_key(k);
            if(pub_mtllib_key == NULL) {
				ldns_rr_free(pubkey);
				return NULL;
            }
            size = pub_mtllib_key->public_key_len;
            bin = LDNS_XMALLOC(unsigned char, size);
            if (!bin) {
                ldns_rr_free(pubkey);
                return NULL;
            }
            memcpy(bin, pub_mtllib_key->public_key, size);
            internal_data = 1;
			break;
#endif
