...
#if defined (PQC_ALGO_MTL) || defined (PQC_ALGO_FL_DSA) || defined (PQC_ALGO_ML_DSA) || defined (PQC_ALGO_SLH_DSA_SHA2) || defined (PQC_ALGO_SLH_DSA_SHAKE) || defined (PQC_ALGO_MAYO_1) || defined(PQC_ALGO_MAYO_2) || defined (PQC_ALGO_SNOVA)
#include <oqs/sig.h>
#endif
 ...
 */
ldns_rdf *
ldns_sign_public_mtl(ldns_buffer *to_sign, MTLLIB_CTX *key) {
    ldns_rdf *sigdata_rdf = NULL;
	MTL_HANDLE* handle = NULL;
	MTLLIB_BUFFER* sign_buffer = NULL;

	if(mtllib_buffer_initialize(&sign_buffer, ldns_buffer_position(to_sign), (unsigned char*)ldns_buffer_begin(to_sign)) != MTLLIB_OK)
	{
		return NULL;
	}
	if( mtllib_sign_append(key, sign_buffer, &handle) != MTLLIB_OK) {
		mtllib_buffer_free(sign_buffer);
		return NULL;
	}
	mtllib_buffer_free(sign_buffer);

	if(handle != NULL) {
	    sigdata_rdf = ldns_rdf_new_frm_data(LDNS_RDF_TYPE_B64, sizeof(MTL_HANDLE), (unsigned char*)handle);
        ...
        if(ldns_rr_get_type(rr_ptr) == LDNS_RR_TYPE_RRSIG) {
            ...
            switch(algo) {
				...
				case LDNS_SIGN_SNOVA_24_5_4_MTL_SHAKE_128:
                    ...
                    ldns_rdf2wire((uint8_t**)&mtl_rrsig_id, sigrdf, &mtl_rrsig_id_size);
                        ...
                        if((priv_key != NULL) &&
                           (
                            ...
                            if(memcmp(mtl_rrsig_id->sid, mtl_ctx->sid.id, mtl_rrsig_id->sid_len) == 0) { 
								...
								// This key matches the MTL context.  Use mtllib_sign_get_condensed_sig()
								...
								if(!full_signature) {
									...
									if(mtllib_sign_get_condensed_sig(priv_key, mtl_rrsig_id, signature_buffer) != MTLLIB_OK) {
										mtllib_buffer_free(signature_buffer);										
										return LDNS_STATUS_INTERNAL_ERR;
									}
									ldns_buffer_write(sign_buf, signature_buffer->buffer_data, signature_buffer->buffer_position);
									mtllib_buffer_free(signature_buffer);
								} else {
									ldns_buffer_write_u8(sign_buf, 1);
									if(mtllib_buffer_initialize(&signature_buffer, mtllib_sign_get_full_sig_length(priv_key, mtl_rrsig_id), NULL) != MTLLIB_OK) {
										return LDNS_STATUS_INTERNAL_ERR;
									}
									if(mtllib_sign_get_full_sig(priv_key, mtl_rrsig_id, signature_buffer) != MTLLIB_OK) {
										mtllib_buffer_free(signature_buffer);										
										return LDNS_STATUS_INTERNAL_ERR;
									}
									ldns_buffer_write(sign_buf, signature_buffer->buffer_data, signature_buffer->buffer_position);
					...
					mtllib_sign_free_handle(&mtl_rrsig_id);
                    break;
                default:
                    break;
            }
